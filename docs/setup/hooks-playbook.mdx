...
sources:
  - claudefa.st/blog/tools/hooks/hooks-guide.md
...
<Tip>**Official reference** — For the full lifecycle event taxonomy, exit code semantics, matcher syntax, and HTTP/agent hooks, see the [official hooks docs](https://code.claude.com/docs/en/hooks). Below is the quick map of what's available.</Tip>
...
**Exit codes:** `0` = allow, `1` = block the action, `2` = force Claude to continue (useful for Stop hooks that enforce quality gates).
