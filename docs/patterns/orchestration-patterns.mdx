...
**Key constraint:** tasks must have no shared state and clear file boundaries. If agents edit overlapping files without [worktree isolation](parallel-execution#worktree-isolation), they'll overwrite each other.
...
**How it works:**
1. Enable [delegate mode](agent-teams#delegate-mode) so the lead can only coordinate
