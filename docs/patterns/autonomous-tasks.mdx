...
sources:
  ...
  - proposals/everything-claude-code.md
---

## The Write-Test Cycle

The most reliable autonomous pattern: Claude writes code, runs tests, checks output, iterates until green. For more examples, see the [official common workflows](https://code.claude.com/docs/en/common-workflows).
...
The Write-Test Cycle works for minutes. For hours-long autonomous runs, you need more structure.

**Verification-first architecture:** Define the success condition before the agent starts. The agent loops on implementation until verification passes — not until it "feels done."
...
- **Stop hooks** can enforce completion conditions — the agent can't finish until tests pass or tasks are marked done. See the [hooks playbook](../setup/hooks-playbook/).
...
<Tip>**Scheduled tasks** in Claude Desktop can run agents on a cadence (hourly, daily) — useful for monitoring, audits, and recurring maintenance without interactive sessions.</Tip>
